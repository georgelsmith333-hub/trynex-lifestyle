...
app.post("/admin/login", async (c) => {
  try {
    ...
    if (!isArgon2Hash(admin.passwordHash)) {
      ...
      await db.update(adminTable).set({ passwordHash: newHash }).where(eq(adminTable.id, admin.id)).catch(() => {});
    }
    if (admin.totpEnabled && admin.totpSecret) {
      const partial = await signPartialTotpToken(admin.id, c.env.ADMIN_JWT_SECRET || c.env.JWT_SECRET);
      return c.json({ requires2FA: true, partialToken: partial }, 200);
    }
...
app.get("/admin/me", requireAdmin, async (c) => {
  try {
    ...
    if (!session.adminId) return c.json({ id: null, username: "system", role: session.role });
    ...
    return c.json({ id: admin.id, username: admin.username, totpEnabled: admin.totpEnabled, role: session.role });
  } catch (err) {
...
app.post("/admin/reset-password", async (c) => {
  try {
    ...
    const newHash = await hashPasswordArgon2(newPassword);
    await db.update(adminTable).set({ passwordHash: newHash, totpEnabled: false, totpSecret: null }).where(eq(adminTable.id, admin.id));
    await revokeAllAdminSessions(db);
