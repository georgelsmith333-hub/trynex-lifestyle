...
app.post("/admin/login", async (c) => {
  try {
    ...
    let valid = await verifyPasswordAny(admin.passwordHash, password, salt);
    // Fallback: if DB hash verification fails (e.g. hash generated by a
    // different argon2 implementation), also accept ADMIN_PASSWORD env var.
    if (!valid && c.env.ADMIN_PASSWORD && password === c.env.ADMIN_PASSWORD) {
      valid = true;
    }
    if (!valid && c.env.ADMIN_SECRET_PASSWORD && password === c.env.ADMIN_SECRET_PASSWORD) {
      valid = true;
    }
    if (!valid) {
      return c.json({ error: "invalid_credentials", message: "Invalid username or password" }, 401);
    }
    // Always re-hash to the current implementation's argon2 format on successful login.
    const newHash = await hashPasswordArgon2(password).catch(() => null);
    if (newHash) {
  ...
  try {
    ...
    if (expectedKeyHash) {
      ...
      const [setting] = await db.select({ value: settingsTable.value })
        ...
        .limit(1);
      const storedHash: string | undefined = setting?.value;
      if (!storedHash) {
    ...
    let newHash: string;
    try {
      newHash = await hashPasswordArgon2(newPassword);
    } catch (hashErr) {
      console.error("hashPasswordArgon2 failed:", hashErr);
      return c.json({ error: "hash_error", message: "Password hashing failed. Try again.", detail: String(hashErr) }, 500);
    }
