 ...
 *   SMTP_PASS     Gmail App Password or provider password
   *   SMTP_FROM     e.g. "Trynext Lifestyle <hello@trynexshop.com>"
 *
...
function fromAddress(): string {
  return process.env.SMTP_FROM ?? `Trynext Lifestyle <${process.env.SMTP_USER ?? "noreply@trynexshop.com"}>`;
}
        ...
        <tr>
          <td style="background:${BRAND_LIGHT};padding:20px 32px;text-align:center;border-top:1px solid #FFE0CC;">
            ...
            <p style="margin:0;font-size:12px;color:#94a3b8;">
              <a href="https://trynex-lifestyle-shop.pages.dev" style="color:${BRAND_ORANGE};text-decoration:none;">trynex-lifestyle-shop.pages.dev</a>
              &nbsp;·&nbsp; Dhaka, Bangladesh
            </p>
          </td>
        </tr>
...
export async function sendOrderConfirmationEmail(order: OrderEmailData): Promise<void> {
  ...
  const body = `
    ...
    <div style="text-align:center;margin-top:28px;">
      <a href="https://trynex-lifestyle-shop.pages.dev/track?order=${order.orderNumber}&phone=${encodeURIComponent(order.customerPhone)}"
