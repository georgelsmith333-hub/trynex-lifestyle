...
export default defineConfig({
  ...
  optimizeDeps: {
    // Exclude large 3D packages and TipTap editor packages from Vite's
    // dep pre-bundler. The 3D libs are lazy-loaded only in the Design Studio
    // route; TipTap is only used in the Admin blog editor. Pre-bundling them
    // causes peer-dep version errors with esbuild.
    exclude: [
      "three",
      "@react-three/fiber",
      "@react-three/drei",
