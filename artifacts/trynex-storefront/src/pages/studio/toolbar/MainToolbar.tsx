...
export function MainToolbar({ onExport }: MainToolbarProps) {
  ...
  return (
    <div className="no-scrollbar w-full overflow-x-auto rounded-2xl border border-gray-200 bg-white shadow-sm">
      <div className="flex min-w-max items-center gap-1.5 p-2">
        ...
        <button type="button" onClick={onExport} disabled={!onExport} title="Export design as PNG" className="flex shrink-0 items-center gap-1.5 rounded-xl bg-gray-900 px-2.5 py-2 text-[10px] font-bold text-white transition hover:bg-gray-800 active:scale-95 disabled:cursor-not-allowed disabled:opacity-50"><Download className="h-3.5 w-3.5" /> Export</button>
      </div>
    </div>
  );
}
