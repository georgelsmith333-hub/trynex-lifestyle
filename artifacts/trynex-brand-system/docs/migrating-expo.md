...
Read `artifacts/trynex-brand-system/docs/AGENTS.md` and
`artifacts/trynex-brand-system/docs/consuming-expo.md` first. Use this guide
...
- Rewrite color/theme imports to
  `@workspace/trynex-brand-system/lib/native-theme` and
  `@workspace/trynex-brand-system/hooks/use-colors`.
- Rewrite the root layout to use
  `@workspace/trynex-brand-system/hooks/use-fonts` while preserving its
  SplashScreen gating.
...
and hook directly from `@workspace/trynex-brand-system` and retains only
